<template>
  <div class="app-container">
    <el-collapse v-model="defaultCollapse" accordion>
      <el-collapse-item title="Add New Record" name="newRecord">
        <NewRecord />
      </el-collapse-item>
    </el-collapse>
          ...
          <span>{{ row.id }}</span>
           ...
           <el-table-column label="Title" min-width="150px">
             <template v-slot="{row}">
               ...
               <el-tag>{{ rowType(row.type) }}</el-tag>
             </template>
           </el-table-column>
           ...
           <el-table-column label="Status" class-name="status-col" width="100">
             <template v-slot="{row}">
               <el-tag :type="rowStatus(row.status)">
                 {{ row.status }}
               </el-tag>
             </template>
           </el-table-column>
...
<script>
import { fetchList, fetchPv, createArticle, updateArticle } from '@/api/article1'
import waves from '@/directives/waves' // waves directive
import { parseTime } from '@/utils'
import Pagination from '@/components/Pagination'
import NewRecord from "@/components/NewRecord"; // secondary package based on el-pagination
...
export default {
  name: 'ComplexTable',
  components: {NewRecord, Pagination },
  directives: { waves },
  computed: {
    statusFilter(status) {
      const statusMap = {
